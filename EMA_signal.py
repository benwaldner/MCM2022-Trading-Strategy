...
def simple_moving_average_signal(df, para=[20, 120]) -> pd.DataFrame:
    ...
    df['ma_short'] = df['close'].ewm(span=ma_short, min_periods=1).mean()
    df['ma_long'] = df['close'].ewm(span=ma_long, min_periods=1).mean()
...
def tune_parameter(gold, bitcoin):
    ...
    # rtn1.to_csv('./result/EMA_signal_gold.csv', index=True, header=True)
    ...
    # rtn2.to_csv('./result/EMA_signal_bitcoin.csv', index=True, header=True)
...
if __name__ == '__main__':
    ...
    # save to csv file
    gold_best.to_csv("./result/EMA_gold_best.csv", index=True, header=True)
    bitcoin_best.to_csv("./result/EMA_bitcoin_best.csv", index=True, header=True)
